...
var _ = Describe(`GlobalTaggingV1`, func() {
	...
	Describe(`SetTags(setTagsOptions *SetTagsOptions) - Operation response error`, func() {
		setTagsPath := "/v3/tags/set_tags"
		Context(`Using mock server endpoint`, func() {
			BeforeEach(func() {
				testServer = httptest.NewServer(http.HandlerFunc(func(res http.ResponseWriter, req *http.Request) {
					defer GinkgoRecover()
					...
					// Verify the contents of the request
					Expect(req.URL.EscapedPath()).To(Equal(setTagsPath))
					Expect(req.Method).To(Equal("POST"))
					Expect(req.Header["X-Ghost"]).ToNot(BeNil())
					Expect(req.Header["X-Ghost"][0]).To(Equal(fmt.Sprintf("%v", "testString")))
					Expect(req.URL.Query()["account_id"]).To(Equal([]string{"testString"}))
			...
			It(`Invoke SetTags with error: Operation response processing error`, func() {
				globalTaggingService, serviceErr := globaltaggingv1.NewGlobalTaggingV1(&globaltaggingv1.GlobalTaggingV1Options{
					URL:           testServer.URL,
					Authenticator: &core.NoAuthAuthenticator{},
				})
				...
				// Construct an instance of the SetTagsOptions model
				setTagsOptionsModel := new(globaltaggingv1.SetTagsOptions)
				setTagsOptionsModel.TagNames = []string{"testString"}
				setTagsOptionsModel.Resources = []globaltaggingv1.Resource{*resourceModel}
				setTagsOptionsModel.XGhost = core.StringPtr("testString")
				setTagsOptionsModel.AccountID = core.StringPtr("testString")
				setTagsOptionsModel.Headers = map[string]string{"x-custom-header": "x-custom-value"}
				// Expect response parsing to fail since we are receiving a text/plain response
				result, response, operationErr := globalTaggingService.SetTags(setTagsOptionsModel)
				Expect(operationErr).ToNot(BeNil())
				Expect(response).ToNot(BeNil())
				Expect(result).To(BeNil())
			})
	...
	Describe(`SetTags(setTagsOptions *SetTagsOptions)`, func() {
		setTagsPath := "/v3/tags/set_tags"
		Context(`Using mock server endpoint`, func() {
			BeforeEach(func() {
				testServer = httptest.NewServer(http.HandlerFunc(func(res http.ResponseWriter, req *http.Request) {
			...
			It(`Invoke SetTags successfully`, func() {
				globalTaggingService, serviceErr := globaltaggingv1.NewGlobalTaggingV1(&globaltaggingv1.GlobalTaggingV1Options{
					URL:           testServer.URL,
					Authenticator: &core.NoAuthAuthenticator{},
				})
				...
				// Invoke operation with nil options model (negative test)
				result, response, operationErr := globalTaggingService.SetTags(nil)
				Expect(operationErr).NotTo(BeNil())
				Expect(response).To(BeNil())
				Expect(result).To(BeNil())
			...
			It(`Invoke SetTags with error: Operation validation and request error`, func() {
				globalTaggingService, serviceErr := globaltaggingv1.NewGlobalTaggingV1(&globaltaggingv1.GlobalTaggingV1Options{
					URL:           testServer.URL,
					Authenticator: &core.NoAuthAuthenticator{},
				})
				...
				Expect(response).To(BeNil())
				Expect(result).To(BeNil())
				// Construct a second instance of the SetTagsOptions model with no property values
				setTagsOptionsModelNew := new(globaltaggingv1.SetTagsOptions)
				// Invoke operation with invalid model (negative test)
				result, response, operationErr = globalTaggingService.SetTags(setTagsOptionsModelNew)
				Expect(operationErr).ToNot(BeNil())
				Expect(response).To(BeNil())
				Expect(result).To(BeNil())
			})
	...
	Describe(`Model constructor tests`, func() {
		Context(`Using a service client instance`, func() {
			...
			It(`Invoke NewSetTagsOptions successfully`, func() {
				// Construct an instance of the Resource model
				resourceModel := new(globaltaggingv1.Resource)
				Expect(resourceModel).ToNot(BeNil())
				resourceModel.ResourceID = core.StringPtr("testString")
				...
				// Construct an instance of the SetTagsOptions model
				setTagsOptionsTagNames := []string{"testString"}
				setTagsOptionsResources := []globaltaggingv1.Resource{}
				setTagsOptionsModel := globalTaggingService.NewSetTagsOptions(setTagsOptionsTagNames, setTagsOptionsResources)
				setTagsOptionsModel.SetTagNames([]string{"testString"})
				setTagsOptionsModel.SetResources([]globaltaggingv1.Resource{*resourceModel})
				setTagsOptionsModel.SetXGhost("testString")
				setTagsOptionsModel.SetAccountID("testString")
				setTagsOptionsModel.SetHeaders(map[string]string{"foo": "bar"})
				Expect(setTagsOptionsModel).ToNot(BeNil())
				Expect(setTagsOptionsModel.TagNames).To(Equal([]string{"testString"}))
				Expect(setTagsOptionsModel.Resources).To(Equal([]globaltaggingv1.Resource{*resourceModel}))
				Expect(setTagsOptionsModel.XGhost).To(Equal(core.StringPtr("testString")))
				Expect(setTagsOptionsModel.AccountID).To(Equal(core.StringPtr("testString")))
				Expect(setTagsOptionsModel.Headers).To(Equal(map[string]string{"foo": "bar"}))
			})
		})
	})
