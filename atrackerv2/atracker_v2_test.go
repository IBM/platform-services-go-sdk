		...
		Context(`Using mock server endpoint with timeout`, func() {
			BeforeEach(func() {
				testServer = httptest.NewServer(http.HandlerFunc(func(res http.ResponseWriter, req *http.Request) {
					...
					res.Header().Set("Content-type", "application/json")
					res.WriteHeader(200)
					fmt.Fprintf(res, "%s", `{"progress": 8, "status": "pending", "migration_items": [{"resource_type": "target", "id": "c3af557f-fb0e-4476-85c3-0889e7fe7bc4", "region": "us-south", "status": "pending", "detailed_status": ["migration of this resource is successful."], "error": "Error"}]}`)
				}))
			})
